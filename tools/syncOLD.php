...
foreach ( $dir  as  $fileinfo ) {
    if (! $fileinfo -> isDot ()) {
        if( strpos ( $fileinfo -> getFilename (),  "udp1194" ) !==  false ) {
             ...
             $req  =  $bdd -> prepare ( "SELECT * FROM radservers WHERE ip=?" );
             $req -> execute (array( $hostname ));
            if( $req -> rowCount ()) {
                 $info  =  $req -> fetch ();
                 $bdd -> prepare ( "DELETE FROM radservers WHERE ID=?" )-> execute (array($info['ID']));
            }
             $bdd -> prepare ( "INSERT INTO radservers VALUES('', :hostname, :auth, :name, :file, 1, 0)" )
                -> execute (array(
...
$bdd->exec("INSERT INTO radserverversion VALUES('', 'Auto')");
